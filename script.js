document.addEventListener('DOMContentLoaded', function() {
    ...
    function findMovieRecommendation(answers) {
        const suitableMovies = movieDatabase.filter(movie => 
    ...
    function showTestResult(recommendation) {
        ...
        recommendedMovieElement.querySelector('.add-to-favorites-btn').addEventListener('click', (e) => {
        ...
        setTimeout(() => {
            ...
            if (results.length === 0) {
                ...
                results.forEach(movie => {
                    const card = createMovieCardFromDatabase(movie);
                    grid.appendChild(card);
                });
            }
            showSection('search');
    ...
    function createMovieCard(film, useKinopoiskId = false) {
        ...
        card.innerHTML = `
            ${posterUrl ? 
                `<img src="${posterUrl}" alt="${film.nameRu || film.nameEn}" class="movie-poster">` :
                `<div class="movie-poster" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); display: flex; align-items: center; justify-content: center; color: white; font-size: 48px;">🎬</div>`
            }
            <div class="movie-info">
    ...
    async function showMovieDetails(filmId) {
        ...
        details.innerHTML = `
            <div class="poster">
                <img src="${filmData.posterUrl}" alt="${filmData.nameRu || filmData.nameEn}">
            </div>
            <div class="info">
